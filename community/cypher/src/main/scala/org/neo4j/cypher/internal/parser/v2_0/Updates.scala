...
trait Updates extends Base with Expressions with StartAndCreateClause with LabelLiteral {
  ...
  private def labelAction: Parser[UpdateAction] = (ignoreCase("add") | ignoreCase("remove")) ~ identity ~ labelList ^^ {

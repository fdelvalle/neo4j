...
class LabelsAcceptanceTest extends ExecutionEngineHelper with StatisticsChecker with Assertions with CollectionSupport {
  ...
  @Test def Removing_a_single_literal_label() {
    usingLabels("FOO", "BAR").
      assertThat("START node=node({node}) REMOVE n LABEL :FOO RETURN n").
      returnsLabels("BAR")
